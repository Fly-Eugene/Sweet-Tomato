  ...
  <div class="modal-mask">
    <div class="modal-container">
      ...
      <header class="select_open">
        <div v-if="state.study_security === 'public'" class="select_open_btn_public" @click="onClickSecurity"><i class="fas fa-lock-open fa-2x"></i></div>
      ...
      <section class="make_study_content">
        <article class="card_left">
          ...
          <input @change="upload" accept="image/*" type="file" id="file" name="study_profile"/>
        </article>
        <article class="card_right">
          ...
          <div class="hash_tag_content">
            <div class="hash_tag_item" v-for="hash in hash_tag_list" :key="hash">{{ hash }}</div>
          </div>
      ...
      <button class="make_study_btn" @click="onClickMakeStudy"><p>스터디 만들기</p></button>    
    </div>
  </div>
</template>
